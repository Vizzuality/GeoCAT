        ...
        edit_metadata = new MetadataInfowindow(new google.maps.LatLng(0,0), null, map);
		      ...
		      fillOpacity: 0
				...
				google.maps.event.addListener(map,"mouseover",function(event){
						switch(state) {
							case 'add': 			map.setOptions({draggableCursor: "url(editor/add_cursor.png),default"});
																break;
							case 'selection': map.setOptions({draggableCursor: "url(editor/selection_cursor.png),default"});
																break;
							case 'remove': 		map.setOptions({draggableCursor: "url(editor/remove_cursor.png),default"});
																break;
							default: 					map.setOptions({draggableCursor: "url(editor/default_cursor.png),default"});
						}
				});
			...
			function showMamufasMap() {
				$('#mamufas_map').css('background','url(editor/mamufas_bkg.png) repeat 0 0');
				$('#mamufas_map').fadeIn();
				$('#loader_map').fadeIn();
			}
